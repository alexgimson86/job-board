...
router.post('/jobs/search/:searchMethod', (req, res) => {
  let searchMethod = req.params.searchMethod
  var searchString = req.body.searchString
  if (searchMethod === "title") {
    Job.find({ 'title': { '$regex': searchString, '$options': 'i' } }, (err, jobs) => {
      res.json(jobs);
    })
  }
  else if (searchMethod === "description") {
    Job.find({ 'jobDescription': { '$regex': searchString, '$options': 'i' } }, (err, jobs) => {
      res.json(jobs);
    })
  }
  else if (searchMethod === "location") {
    Job.find({ 'location': { '$regex': searchString, '$options': 'i' } }, (err, jobs) => {
      res.json(jobs);
    })
  }
})
